...
class HumanCharacter(interfaces.ICharacter):
    def __init__(self, board: interfaces.IBoard, character: model.characters.HumanCharacter, color: interfaces.RGB):
        ...
        self.__arrow: pygame.Surface = pygame.Surface((self._board.TileWidth * 0.33333, self._board.TileHeight * 0.33333))
        self.__still: pygame.Surface = pygame.Surface((self._board.TileWidth * 0.33333, self._board.TileHeight * 0.33333))

        size: typing.Tuple[int, int] = self.__arrow.get_size()
        ...
        pygame.draw.polygon(self.__arrow, self._color, points)

        points = (
            pygame.Vector2(0, 0),
            pygame.Vector2(size[0], 0),
            pygame.Vector2(size[0], size[1]),
            pygame.Vector2(0, size[1])
        )

        pygame.draw.polygon(self.__still, self._color, points)


    def render(self, screen: pygame.Surface) -> None:
        if not self._moved or self.Dead:
            image: pygame.Surface = self.__still
            size: typing.Tuple[int, int] = self.__still.get_size()
        else:    
            image: pygame.Surface = pygame.transform.rotate(self.__arrow, self._theta)
...
class NEATTraineeCharacter(interfaces.ICharacter):
    def __init__(self, board: interfaces.IBoard, character: model.characters.NEATTraineeCharacter, color: interfaces.RGB):
        interfaces.ICharacter.__init__(self, board, character, color)
        self.__arrow: pygame.Surface = pygame.Surface((self._board.TileWidth * 0.33333, self._board.TileHeight * 0.33333))
        self.__still: pygame.Surface = pygame.Surface((self._board.TileWidth * 0.33333, self._board.TileHeight * 0.33333))
    ...
    def _do_step(self, verbose: bool = False) -> typing.Tuple[int, int]:
        decision: typing.Optional[bool] = typing.cast(model.characters.NEATTraineeCharacter, self._character).react()
        move: typing.Tuple[int, int] = self._character.do_step(decision, verbose)
        return move
...
class CharactersFactory:
    ...
    def create_character(idx: int, board: interfaces.IBoard, character: model.interfaces.ICharacter, color: interfaces.RGB) -> interfaces.ICharacter:
        ...
        elif isinstance(character, model.characters.NEATTraineeCharacter):
