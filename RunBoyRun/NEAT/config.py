...
neat.config
class Config(object):
    def __init__(self, pop_size: int, fitness_criterion: str, fitness_threshold: float, reset_on_extinction: bool, no_fitness_termination: bool = False) -> None:
        self.pop_size: int = pop_size
        self.fitness_criterion: str = fitness_criterion
        self.fitness_threshold: float = fitness_threshold
        self.reset_on_extinction: bool = reset_on_extinction
        self.no_fitness_termination: bool = no_fitness_termination
...
StracturealMutationSurerLiterals = typing.Literal['0', '1', 'yes', 'no', 'true', 'false','on', 'off', 'default']
ConnectionLiterals = typing.Literal['unconnected', 'fs_neat_nohidden', 'fs_neat', 'fs_neat_hidden', 'full_nodirect', 'full', 'full_direct', 'partial_nodirect', 'partial', 'partial_direct']
...
class DefaultGenomeConfig(object):
    ...
    def __init__(self, num_inputs: int, num_outputs: int, num_hidden: int, feed_forward: bool,
                       ...
                       structural_mutation_surer: StracturealMutationSurerLiterals = 'default',
                       initial_connection: ConnectionLiterals = 'unconnected',
                       **wargs) -> None:
        ...
        self.compatibility_disjoint_coefficient: float = compatibility_disjoint_coefficient
        self.compatibility_weight_coefficient: float = compatibility_weight_coefficient
        self.conn_add_prob: float = conn_add_prob
        self.conn_delete_prob: float = conn_delete_prob
        self.node_add_prob: float = node_add_prob
        self.node_delete_prob: float = node_delete_prob
        self.single_structural_mutation: bool = single_structural_mutation
        self.structural_mutation_surer: StracturealMutationSurerLiterals = structural_mutation_surer
        self.initial_connection: ConnectionLiterals = initial_connection
        self.connection_fraction: typing.Optional[float] = None
        # By convention, input pins have negative keys, and the output
        # pins have keys 0,1,...
        self.input_keys: typing.List[int] = [-i - 1 for i in range(self.num_inputs)]
        self.output_keys: typing.List[int] = [i for i in range(self.num_outputs)]
        self.node_indexer: typing.Optional[itertools.count] = None
    
        self._params: typing.List[neat.config.ConfigParameter] = []
        
        # Create full set of available activation functions.
        self.activation_defs = neat.activations.ActivationFunctionSet()
        # ditto for aggregation functions - name difference for backward compatibility
        self.aggregation_function_defs = neat.aggregations.AggregationFunctionSet()
        self.aggregation_defs = self.aggregation_function_defs

        # Gather configuration data from the gene classes.
        self.node_gene_type = neat.genes.DefaultNodeGene
        self._params += self.node_gene_type.get_config_params()
        self.connection_gene_type = neat.genes.DefaultConnectionGene
        self._params += self.connection_gene_type.get_config_params()

        # Use the configuration data to interpret the supplied parameters.
        for p in self._params:
            setattr(self, p.name, p.interpret(wargs))
        ...
        assert self.initial_connection in typing.get_args(ConnectionLiterals)
    ...
    def get_new_node_key(self, node_dict: typing.Dict[typing.Any, int]) -> int:
        if self.node_indexer is None:
            self.node_indexer = itertools.count(max(list(neat.six_util.iterkeys(node_dict))) + 1)
